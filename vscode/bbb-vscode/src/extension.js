...
const { formatBBB } = require('./formatter'); // JS fallback
...
function activate(context) {
  ...
  const provider = vscode.languages.registerCompletionItemProvider('bbb', {
...
<script>
  ...
  const runFile = vscode.commands.registerCommand('bbb.runFile', async () => {
    const doc = vscode.window.activeTextEditor && vscode.window.activeTextEditor.document;
    if (!doc) { vscode.window.showWarningMessage('No file is open'); return; }
    if (doc.languageId !== 'bbb') { vscode.window.showWarningMessage('The active file is not BBB (.bl/.bio)'); return; }
    await doc.save();
    const fileName = doc.fileName;
    const panel = vscode.window.createWebviewPanel(
      'bbbRun', `BBB — ${path.basename(fileName)}`, vscode.ViewColumn.One,
    ...
    const child = spawn('bbb', [fileName], { cwd });
    let killed = false;
    ...
    child.on('error', e => panel.webview.postMessage({ type: 'err', text: '[bbb] ' + e.message + '\n' }));
    child.on('close', code => {
  ...
  /* Compile current file: bbb shell build file → self-contained executable (output into workspace bin/) */
  const compileFile = vscode.commands.registerCommand('bbb.compileFile', async () => {
    const doc = vscode.window.activeTextEditor && vscode.window.activeTextEditor.document;
    if (!doc) { vscode.window.showWarningMessage('No file is open'); return; }
    if (doc.languageId !== 'bbb') { vscode.window.showWarningMessage('The active file is not BBB (.bl/.bio)'); return; }
    await doc.save();
    const out = vscode.window.createOutputChannel('BBB Compile');
    out.show(true);
    ...
    out.appendLine(`$ bbb shell build ${doc.fileName} -o ${outBin}`);
    execFile('bbb', ['shell', 'build', doc.fileName, '-o', outBin], { cwd }, (err, stdout, stderr) => {
  ...
  function projCmd(cmd, args, successMsg, isRun) {
    ...
    const out = vscode.window.createOutputChannel('BBB Project');
    out.show(true);
    out.appendLine(`$ bbb ${cmd} ${dir}${args ? ' ' + args : ''}`);
    if (isRun) {
      /* project run: Webview interactive (stdin input) */
      const panel = vscode.window.createWebviewPanel('bbbProjRun', `BBB Project Run`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
      panel.webview.html = panelHtml('Project run (bio run)');
      const child = spawn('bbb', ['run', dir], { cwd: dir });
      let killed = false;
      child.stdout.on('data', d => panel.webview.postMessage({ type: 'out', text: d.toString() }));
      child.stderr.on('data', d => panel.webview.postMessage({ type: 'err', text: d.toString() }));
      child.on('error', e => panel.webview.postMessage({ type: 'err', text: '[bbb] ' + e.message + '\n' }));
    ...
    execFile('bbb', [cmd, dir].concat(args ? args.split(' ') : []), { cwd: dir }, (err, stdout, stderr) => {
      if (stdout) out.append(stdout);
      if (stderr) out.append(stderr);
  ...
  const projectInit = vscode.commands.registerCommand('bbb.projectInit', async () => {
    ...
    execFile('bbb', ['init', ws ? path.join(ws, name) : name], {}, (err, stdout, stderr) => {
      if (stdout) out.append(stdout);
      if (stderr) out.append(stderr);
  ...
  const projectBuild = vscode.commands.registerCommand('bbb.projectBuild', () => {
    const dir = projectRoot();
    if (!dir) { vscode.window.showWarningMessage('No project found (package.toml)'); return; }
    const out = vscode.window.createOutputChannel('BBB Project');
    out.show(true);
    out.appendLine(`$ bbb build ${dir}`);
    execFile('bbb', ['build', dir, '-o', path.join(dir, 'app')], { cwd: dir }, (err, stdout, stderr) => {
      if (stdout) out.append(stdout);
      if (stderr) out.append(stderr);
  ...
  const projectRun = vscode.commands.registerCommand('bbb.projectRun', () => projCmd('run', null, null, true));

  const projectInstall = vscode.commands.registerCommand('bbb.projectInstall', () => projCmd('install', null, '✔ dependencies installed'));

  const projectDestroy = vscode.commands.registerCommand('bbb.projectDestroy', async () => {
    const dir = projectRoot();
    if (!dir) { vscode.window.showWarningMessage('No project found (package.toml)'); return; }
    ...
    const out = vscode.window.createOutputChannel('BBB Project');
    out.show(true);
    execFile('bbb', ['destroy', dir], { cwd: dir }, (err, stdout, stderr) => {
      if (stdout) out.append(stdout);
      if (stderr) out.append(stderr);
  ...
  const runDemo = vscode.commands.registerCommand('bbb.runDemo', () => {
    const out = vscode.window.createOutputChannel('BBB');
    out.show(true);
    out.appendLine('$ bbb');
    execFile('bbb', [], {}, (err, stdout, stderr) => {
      if (stdout) out.append(stdout);
      if (stderr) out.append(stderr);
  ...
  loadWasmFormatter();
  const formatter = vscode.languages.registerDocumentFormattingProvider('bbb', {
    provideDocumentFormattingEdits(document) {
      ...
      if (formatted === null || formatted === undefined) {
        formatted = formatBBB(text); // JS fallback
      }
