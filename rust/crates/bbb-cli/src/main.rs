...
fn cmd_run(arg: &str) -> Result<(), String> {
    ...
    let (prog, unmet) = if path.is_dir() {
        let prog = bbb_vm::load_project_sources(&path)?;
    ...
    let unmet = unmet
        .into_iter()
        .filter(|(k, n)| !(k == "value" && prog.decls.iter().any(|d| matches!(d, bbb_syntax::Decl::Const { name, .. } if name == n))))
        .collect::<Vec<_>>();
    if !unmet.is_empty() {
        return Err(unmet
            .iter()
...
fn cmd_llvm(arg: &str) -> Result<(), String> {
    ...
    // 1. AST → IR
    let ir = bbb_llvm::compile(&prog).map_err(|e| format!("IR 生成失败: {e}"))?;
    // 2. IR → clang → 可执行
